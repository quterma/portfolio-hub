...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <Container>
        <nav className="flex h-14 w-full items-center gap-2 text-sm font-medium sm:justify-start">
          {navigationItems.map(item => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`group relative flex flex-1 items-center justify-center space-x-1.5 overflow-hidden rounded-md border px-3 py-2 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:flex-initial sm:border-transparent sm:bg-transparent sm:after:absolute sm:after:left-0 sm:after:-bottom-0.5 sm:after:h-[2px] sm:after:w-full sm:after:bg-foreground sm:after:origin-left sm:after:transition-transform sm:after:duration-200 ${
                  isActive
